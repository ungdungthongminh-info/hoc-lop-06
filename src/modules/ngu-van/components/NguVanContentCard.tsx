...
type NguVanContentCardProps = { card: NguVanContentCardData; index: number; total: number; };
  ...
  return (
    <article className={`w-full max-w-full min-w-0 rounded-3xl border p-5 shadow-sm ${getCardTone(card.cardType)}`}>
      <div className="flex min-w-0 flex-wrap items-center justify-between gap-3">
        <p className="inline-flex min-w-0 items-center gap-2 text-xs font-black uppercase tracking-[0.14em] [overflow-wrap:anywhere]"><NotebookText className="h-4 w-4 shrink-0" />Thẻ {index + 1}/{total}</p>
        <span className="rounded-full bg-white/70 px-3 py-1 text-xs font-black text-slate-600 ring-1 ring-black/5">{card.cardType.replace(/_/g, ' ')}</span>
      </div>
