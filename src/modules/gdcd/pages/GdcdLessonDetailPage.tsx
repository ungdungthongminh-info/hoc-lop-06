...
import { getGdcdLessonCards, getGdcdLessonQuestions } from '../../../data/grade6/gdcd';
import { GdcdContentCard } from '../components/GdcdContentCard';
...
export function GdcdLessonDetailPage({ lesson, onBack, onPractice }: Props) {
  const cards = getGdcdLessonCards(lesson.id);
  const questions = getGdcdLessonQuestions(lesson.id);
  ...
  return (
    <section className="mx-auto w-full max-w-5xl min-w-0 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="w-full max-w-full min-w-0 rounded-3xl border border-violet-100 bg-white p-5 shadow-sm">
        <div className="flex min-w-0 flex-col gap-5 md:flex-row md:items-start md:justify-between">
          <div className="min-w-0 max-w-3xl">
            <p className="text-sm font-black uppercase tracking-[0.14em] text-violet-700 [overflow-wrap:anywhere]">{lesson.unitTitle}</p>
          ...
          <button
            ...
            className="inline-flex w-full min-w-0 items-center justify-center gap-2 rounded-2xl bg-violet-600 px-5 py-3 text-sm font-black text-white shadow-lg shadow-violet-100 transition hover:bg-violet-700 md:w-auto md:shrink-0"
          >
        ...
        {keywordEntries.length ? (
          <div className="mt-5 rounded-2xl border border-violet-100 bg-violet-50/60 p-4">
            <p className="text-xs font-black uppercase tracking-[0.12em] text-violet-700">Ghi nhớ từ khóa</p>
            <div className="mt-3 grid min-w-0 gap-2 sm:grid-cols-[repeat(2,minmax(0,1fr))] lg:grid-cols-[repeat(3,minmax(0,1fr))]">
              {keywordEntries.map(([word, meaning]) => (
                <div key={word} className="min-w-0 rounded-2xl bg-white px-3 py-2 text-sm ring-1 ring-violet-100">
      ...
      <div className="mt-6 grid min-w-0 gap-4">
        {cards.map((card, index) => <GdcdContentCard key={card.id} card={card} index={index} total={cards.length} />)}
      </div>

      <div className="mt-6 w-full max-w-full min-w-0 rounded-3xl border border-slate-200 bg-white p-5 text-center shadow-sm">
        <Clock className="mx-auto h-8 w-8 text-violet-700" />
        ...
        <p className="mt-1 text-sm text-slate-600 [overflow-wrap:anywhere]">
          Kho câu hỏi bài này có {questions.length} câu. Em có thể chọn số câu vừa sức rồi đọc giải thích sau mỗi câu.
        </p>
        <button type="button" onClick={() => onPractice(lesson.id)} className="mt-4 inline-flex w-full min-w-0 items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-black text-white transition hover:bg-violet-700 sm:w-auto">
