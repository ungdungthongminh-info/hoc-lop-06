import { ArrowLeft, BookOpenCheck, Clock, FlaskConical, PlayCircle } from 'lucide-react';

import type { KhtnLesson } from '../../../data/grade6/khtn';
import { getKhtnLessonCards, getKhtnLessonQuestions } from '../../../data/grade6/khtn';
import { KhtnContentCard } from '../components/KhtnContentCard';

type KhtnLessonDetailPageProps = {
  lesson: KhtnLesson;
  onBack: () => void;
  onPractice: (lessonId: number) => void;
};

export function KhtnLessonDetailPage({ lesson, onBack, onPractice }: KhtnLessonDetailPageProps) {
  const cards = getKhtnLessonCards(lesson.id);
  const questions = getKhtnLessonQuestions(lesson.id);
  ...
  return (
    <section className="mx-auto w-full max-w-5xl min-w-0 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="w-full max-w-full min-w-0 rounded-3xl border border-emerald-100 bg-white p-5 shadow-sm">
        <div className="flex min-w-0 flex-col gap-5 md:flex-row md:items-start md:justify-between">
          <div className="min-w-0 max-w-3xl">
            <p className="text-sm font-black uppercase tracking-[0.14em] text-emerald-700 [overflow-wrap:anywhere]">
              {lesson.unitTitle}
            </p>
          ...
          <button
            ...
            className="inline-flex w-full min-w-0 items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-5 py-3 text-sm font-black text-white shadow-lg shadow-emerald-200 transition hover:bg-emerald-700 md:w-auto md:shrink-0"
          >
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700">{cards.length} thẻ nội dung</span>
        ...
        {keywordEntries.length ? (
          <div className="mt-5 rounded-2xl border border-emerald-100 bg-emerald-50/60 p-4">
            <p className="text-xs font-black uppercase tracking-[0.12em] text-emerald-700">Khái niệm chính</p>
            <div className="mt-3 grid min-w-0 gap-2 sm:grid-cols-[repeat(2,minmax(0,1fr))] lg:grid-cols-[repeat(3,minmax(0,1fr))]">
              {keywordEntries.map(([word, meaning]) => (
                <div key={word} className="min-w-0 rounded-2xl bg-white px-3 py-2 text-sm ring-1 ring-emerald-100">
      ...
      <div className="mt-6 grid min-w-0 gap-4">
        {cards.map((card, index) => (
          <KhtnContentCard key={card.id} card={card} index={index} total={cards.length} />
        ))}
      </div>

      <div className="mt-6 w-full max-w-full min-w-0 rounded-3xl border border-slate-200 bg-white p-5 text-center shadow-sm">
        <Clock className="mx-auto h-8 w-8 text-emerald-700" />
        ...
        <button
          type="button"
          onClick={() => onPractice(lesson.id)}
          className="mt-4 inline-flex w-full min-w-0 items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-black text-white transition hover:bg-emerald-700 sm:w-auto"
        >
