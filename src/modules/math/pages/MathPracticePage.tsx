...
import type { MathLesson, MathPracticeQuestionCountOption } from '../../../data/grade6/toan';
import {
  DEFAULT_PRACTICE_QUESTION_COUNT,
  PRACTICE_QUESTION_COUNT_OPTIONS,
  getMathLessonPracticeQuestions,
  isPracticeQuestionCountOption,
...
import { getPracticeSummary, isCorrectAnswer } from '../utils/mathPracticeEngine';

const PRACTICE_COUNT_STORAGE_KEY = 'lop6.math.practiceQuestionCount';

type MathPracticePageProps = {
  lesson: MathLesson;
  onBackToLesson: (lessonId: number) => void;
};

function getStoredPracticeQuestionCount(): MathPracticeQuestionCountOption {
  if (typeof window === 'undefined') return DEFAULT_PRACTICE_QUESTION_COUNT;

  const storedValue = Number(window.localStorage.getItem(PRACTICE_COUNT_STORAGE_KEY));
  return isPracticeQuestionCountOption(storedValue) ? storedValue : DEFAULT_PRACTICE_QUESTION_COUNT;
}

export function MathPracticePage({ lesson, onBackToLesson }: MathPracticePageProps) {
  const [questionCount, setQuestionCount] = useState<MathPracticeQuestionCountOption>(() => getStoredPracticeQuestionCount());
  const [practiceRound, setPracticeRound] = useState(0);
  const [practiceStarted, setPracticeStarted] = useState(false);
  const questions = useMemo(
    () => (practiceStarted ? getMathLessonPracticeQuestions(lesson.id, questionCount) : []),
  ...
  const { incrementMathCorrect, markMathStudyToday, unlockBadge } = useBadges();
  const sound = useSound();

  const currentQuestion = practiceStarted ? questions[currentIndex] : undefined;
  const currentAnswer = currentQuestion ? answers[currentQuestion.id] : undefined;
  const summary = getPracticeSummary(questions, answers);
  ...
  const handleQuestionCountChange = (nextCount: MathPracticeQuestionCountOption) => {
  ...
  const handleAnswer = (answer: string) => {
    ...
    if (isCorrect) {
      ...
      toast.success('Đúng rồi!', 'Em chọn chính xác. Tiếp tục giữ nhịp nhé.');
      return;
    }
  ...
  const finishPractice = () => {
    ...
    unlockBadge('math-first-lesson');
    setIsFinished(true);
  };

  const questionCountSelector = (
    <div className="mb-5 w-full max-w-full min-w-0 rounded-3xl border border-blue-100 bg-white p-4 shadow-sm">
      <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          <p className="text-xs font-black uppercase tracking-[0.14em] text-blue-700">Số câu luyện tập</p>
          <p className="mt-1 text-sm font-semibold text-slate-600 [overflow-wrap:anywhere]">
            App sẽ đảo câu hỏi mỗi lượt để tránh học tủ.
          </p>
        </div>
        <span className="max-w-full rounded-full bg-blue-50 px-3 py-1 text-xs font-black text-blue-700 [overflow-wrap:anywhere]">
          Đang chọn {questionCount} câu
        </span>
      </div>

      <div className="mt-4 grid min-w-0 gap-2 sm:grid-cols-[repeat(2,minmax(0,1fr))]">
        {PRACTICE_QUESTION_COUNT_OPTIONS.map((option) => {
      ...
      <section className="mx-auto w-full max-w-3xl min-w-0 px-4 py-8 sm:px-6 lg:px-8">
        <div className="w-full max-w-full min-w-0 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="min-w-0 text-center">
            <p className="text-xs font-black uppercase tracking-[0.14em] text-blue-700">Chuẩn bị luyện tập</p>
            ...
            <p className="mx-auto mt-3 max-w-2xl text-sm font-medium leading-6 text-slate-600 [overflow-wrap:anywhere]">
              Chọn số câu trước rồi mới bắt đầu làm bài. Lựa chọn sẽ được lưu lại trên thiết bị.
            </p>
          </div>
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:justify-center">
            <button
              ...
              className="inline-flex w-full min-w-0 items-center justify-center rounded-2xl bg-blue-600 px-6 py-4 text-sm font-black text-white transition hover:bg-blue-700 sm:w-auto"
            >
              Bắt đầu luyện tập
            </button>
            <button
          ...
          <p className="mt-4 text-xs font-semibold uppercase tracking-[0.12em] text-slate-500 [overflow-wrap:anywhere]">
            Mặc định: 12 câu. 10/12/20 sẽ dùng chính sách chọn ngẫu nhiên, 40 câu là toàn bộ và shuffle.
          </p>
        </div>
      </section>
    );
      ...
      <section className="mx-auto w-full max-w-3xl min-w-0 px-4 py-8 sm:px-6 lg:px-8">
        <div className="w-full max-w-full min-w-0 rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-3 text-lg font-bold text-slate-700 [overflow-wrap:anywhere]">
            Đúng {summary.correctCount}/{summary.totalCount} câu - Sai {summary.wrongCount} câu
          </p>
          <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
            ...
            <button
              ...
              className="inline-flex w-full min-w-0 items-center justify-center gap-2 rounded-2xl bg-blue-600 px-5 py-3 text-sm font-black text-white hover:bg-blue-700 sm:w-auto"
            >
    ...
    <section className="mx-auto w-full max-w-3xl min-w-0 px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-5 grid min-w-0 gap-3 sm:flex sm:flex-wrap sm:items-center sm:justify-between">
        ...
        <div className="grid w-full min-w-0 grid-cols-1 gap-2 sm:flex sm:w-auto sm:flex-wrap sm:items-center">
          <span className="inline-flex min-w-0 items-center justify-center rounded-full bg-blue-50 px-3 py-2 text-sm font-black text-blue-700 sm:py-1">
            {questionCount} câu
          </span>
          <button
            type="button"
      ...
      {currentAnswer ? (
        <div className="mt-5 text-center">
          <button
            ...
            className="inline-flex w-full min-w-0 items-center justify-center gap-2 rounded-2xl bg-slate-950 px-6 py-3 text-sm font-black text-white transition hover:bg-blue-700 sm:w-auto"
          >
