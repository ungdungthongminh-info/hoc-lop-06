...
import type { CongNgheLesson, CongNghePracticeQuestionCountOption } from '../../../data/grade6/cong-nghe';
import {
  CONG_NGHE_PRACTICE_QUESTION_COUNT_OPTIONS,
  DEFAULT_CONG_NGHE_PRACTICE_QUESTION_COUNT,
  getCongNgheLessonPracticeQuestions,
  isCongNghePracticeQuestionCountOption,
} from '../../../data/grade6/cong-nghe';
import { useToast } from '../../../shared/components/ToastProvider';
import { useSound } from '../../../shared/hooks/useSound';
import {
  CongNgheQuestionCard,
  isCongNgheAnswerCorrect,
  isCongNgheReferenceAnswer,
} from '../components/CongNgheQuestionCard';
...
const STORAGE_KEY = 'lop6.congNghe.practiceQuestionCount';

function getStoredQuestionCount(): CongNghePracticeQuestionCountOption {
  if (typeof window === 'undefined') return DEFAULT_CONG_NGHE_PRACTICE_QUESTION_COUNT;
  const value = Number(window.localStorage.getItem(STORAGE_KEY));
  return isCongNghePracticeQuestionCountOption(value) ? value : DEFAULT_CONG_NGHE_PRACTICE_QUESTION_COUNT;
}

export function CongNghePracticePage({ lesson, onBackToLesson }: Props) {
  const [questionCount, setQuestionCount] = useState<CongNghePracticeQuestionCountOption>(() => getStoredQuestionCount());
  const [practiceRound, setPracticeRound] = useState(0);
  const [practiceStarted, setPracticeStarted] = useState(false);
  const questions = useMemo(() => getCongNgheLessonPracticeQuestions(lesson.id, questionCount), [lesson.id, questionCount, practiceRound]);
  ...
  const correctCount = questions.reduce((count, question) => {
    ...
    return isCongNgheAnswerCorrect(question, answer) ? count + 1 : count;
  }, 0);
  ...
  const handleQuestionCountChange = (nextCount: CongNghePracticeQuestionCountOption) => {
  ...
  const handleAnswer = (answer: string) => {
    ...
    const isReference = isCongNgheReferenceAnswer(answer);
    const right = isCongNgheAnswerCorrect(currentQuestion, answer);
  ...
  const questionCountSelector = (
    <div className="mb-5 w-full max-w-full min-w-0 rounded-3xl border border-orange-100 bg-white p-4 shadow-sm">
      <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
          <p className="text-xs font-black uppercase tracking-[0.14em] text-orange-700">Số câu luyện tập</p>
        ...
        <span className="max-w-full rounded-full bg-orange-50 px-3 py-1 text-xs font-black text-orange-700 [overflow-wrap:anywhere]">
          Đang chọn {questionCount} câu
        </span>
      </div>
      <div className="mt-4 grid min-w-0 gap-2 sm:grid-cols-[repeat(2,minmax(0,1fr))]">
        {CONG_NGHE_PRACTICE_QUESTION_COUNT_OPTIONS.map((option) => {
          ...
          return (
            <button
              ...
              className={`w-full min-w-0 rounded-2xl border p-3 text-left transition ${selected ? 'border-orange-300 bg-orange-50 text-orange-900 shadow-sm' : 'border-slate-200 bg-white text-slate-700 hover:border-orange-200 hover:bg-orange-50'}`}
              aria-pressed={selected}
            >
              <span className="flex min-w-0 flex-wrap items-center justify-between gap-2">
                <span className="min-w-0 text-sm font-black [overflow-wrap:anywhere]">{option.title}</span>
                <span className="shrink-0 rounded-full bg-white px-2.5 py-1 text-xs font-black text-orange-700 ring-1 ring-orange-100">{option.label}</span>
              </span>
      ...
      <section className="mx-auto w-full max-w-3xl min-w-0 px-4 py-8 sm:px-6 lg:px-8">
        <div className="w-full max-w-full min-w-0 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="min-w-0 text-center">
            <p className="text-xs font-black uppercase tracking-[0.14em] text-orange-700">Chuẩn bị luyện tập</p>
            ...
            <p className="mx-auto mt-3 max-w-2xl text-sm font-medium leading-6 text-slate-600 [overflow-wrap:anywhere]">
              Mỗi câu đều có đáp án hoặc gợi ý ngắn để em tự kiểm tra kiến thức, quy trình và tình huống gia đình.
            </p>
          </div>
          {questionCountSelector}
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:justify-center">
            <button type="button" onClick={startPractice} className="inline-flex w-full min-w-0 items-center justify-center rounded-2xl bg-orange-600 px-6 py-4 text-sm font-black text-white transition hover:bg-orange-700 sm:w-auto">
              Bắt đầu luyện tập
            </button>
      ...
      <section className="mx-auto w-full max-w-3xl min-w-0 px-4 py-8 sm:px-6 lg:px-8">
        <div className="w-full max-w-full min-w-0 rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          ...
          <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
            ...
            <button type="button" onClick={startPractice} className="inline-flex w-full min-w-0 items-center justify-center gap-2 rounded-2xl bg-orange-600 px-5 py-3 text-sm font-black text-white hover:bg-orange-700 sm:w-auto">
    ...
    <section className="mx-auto w-full max-w-3xl min-w-0 px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-5 grid min-w-0 gap-3 sm:flex sm:flex-wrap sm:items-center sm:justify-between">
        ...
        <div className="grid w-full min-w-0 grid-cols-1 gap-2 sm:flex sm:w-auto sm:flex-wrap sm:items-center">
          <span className="inline-flex min-w-0 items-center justify-center rounded-full bg-orange-50 px-3 py-2 text-sm font-black text-orange-700 sm:py-1">{questionCount} câu</span>
      ...
      {currentAnswer ? (
        <div className="mt-5 text-center">
          <button
            ...
            className="inline-flex w-full min-w-0 items-center justify-center gap-2 rounded-2xl bg-slate-950 px-6 py-3 text-sm font-black text-white transition hover:bg-orange-700 sm:w-auto"
          >
