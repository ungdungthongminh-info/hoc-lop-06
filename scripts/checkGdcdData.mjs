...
const dataDir = path.join(root, 'src/data/grade6/gdcd');
...
const lessons = readArray('gdcdLessons.ts', 'gdcdLessons');
const cards = readArray('gdcdLessonCards.ts', 'gdcdLessonCards');
const questions = readArray('gdcdQuestions.ts', 'gdcdQuestions');
...
const rigidPattern = /phải luôn luôn|tuyệt đối không được|bắt buộc phải/gi;
const errors = [];
const warnings = [];
...
function checkText(label, value) {
  ...
  const rigidHits = text.match(rigidPattern);
  if (rigidHits?.length) warnings.push(`Rigid wording in ${label}: ${rigidHits.join(', ')}`);
}
